...
public class GitIgnoreHardValidationCheckTest {

    private SensorContext sensorContext;
    private InputFile inputFile;
    private Project project;
    private GitIgnoreSoftValidationCheck check;
    private NewIssue issue;
    private NewIssueLocation location;
    private List<String> patterns = Lists.newArrayList("*.classpath", "*.project", "*.settings");

    @Before
    public void setUp() {
        check = new GitIgnoreSoftValidationCheck();
    ...
    public void analyzeInvalidTest() {
        when(inputFile.file()).thenReturn(new File("src/test/files/git/.gitignore-hard-validation/invalid/.gitignore"));
        ...
        for (String pattern : patterns) {
            verify(location).message(eq(String.format("'%s' should not contain an asterisk (*).", pattern)));
        }
    ...
    public void analyzeValidTest() {
        when(inputFile.file()).thenReturn(new File("src/test/files/git/.gitignore-hard-validation/valid/.gitignore"));
